...
class DbManager {
  ...
  Future openDb() async {
    if (_database == null) {
      _database = await openDatabase(join(await getDatabasesPath(), dbKinopoisk), version: 1,
        ...
        await db.execute(
            "CREATE TABLE $tablePicture (id INTEGER PRIMARY KEY AUTOINCREMENT, poster TEXT, description TEXT)");
        await db.execute(
            "CREATE TABLE $tableFavorites (id INTEGER PRIMARY KEY AUTOINCREMENT, customer INTEGER, name TEXT, type TEXT, title TEXT, year INTEGER, poster TEXT, description TEXT)");
        // await db.execute(
  ...
  //   await _database.execute("INSERT INTO $tableCinema(name, places) VALUES('Мир Люксор', 500)");
  //   await _database.execute("INSERT INTO $tableCinema(name, places) VALUES('Три пингвина', 700)");
  //   await _database.execute("INSERT INTO $tableCinema(name, places) VALUES('Волжский', 350)");
  //   await _database.execute("INSERT INTO $tablePrices(prices) VALUES(250)");
  //   await _database.execute("INSERT INTO $tablePrices(prices) VALUES(300)");
  //   await _database.execute("INSERT INTO $tablePrices(prices) VALUES(150)");
  // }
